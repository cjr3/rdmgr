...
function DataReducer(state = InitState, action) {
    switch(action.type) {
        ...
        case SET_CONFIG_CAPTURE :
            return Object.assign({}, state, {
                CaptureConfig:Object.assign({}, state.CaptureConfig, action.config)
            });
...
const DataController = {
    ...
    CheckFiles() {
        ...
        recordFiles.forEach((filename) => {
            if(!DataController.FS.existsSync(filename)) {
        ...
        if(!DataController.FS.existsSync(FILE_TEAMS)) {
        ...
        //break: 5:00
        //4th quarter: 15:00
        if(!DataController.FS.existsSync(FILE_PHASES)) {
        ...
        //misc records
        if(!DataController.FS.existsSync(FILE_RECORDS)) {
    ...
    CheckConfigFiles() {
        //main config
        if(!DataController.FS.existsSync(FILE_CONFIG)) {
        ...
        //scoreboard state
        if(!DataController.FS.existsSync(FILE_STATE_SCOREBOARD)) {
        ...
        //camera 
        if(!DataController.FS.existsSync(FILE_STATE_CAMERA)) {
        ...
        //capture
        if(!DataController.FS.existsSync(FILE_STATE_CAPTURE)) {
        ...
        //chat
        if(!DataController.FS.existsSync(FILE_STATE_CHAT)) {
        ...
        //penalty tracker
        if(!DataController.FS.existsSync(FILE_STATE_PENALTY)) {
        ...
        //raffle
        if(!DataController.FS.existsSync(FILE_STATE_RAFFLE)) {
        ...
        //roster
        if(!DataController.FS.existsSync(FILE_STATE_ROSTER)) {
        ...
        //scorekeeper
        if(!DataController.FS.existsSync(FILE_STATE_SCOREKEEPER)) {
        ...
        //Slideshow
        if(!DataController.FS.existsSync(FILE_STATE_SLIDESHOW)) {
        ...
        //Sponsors
        if(!DataController.FS.existsSync(FILE_STATE_SPONSOR)) {
        ...
        //Video
        if(!DataController.FS.existsSync(FILE_STATE_VIDEO)) {
     ...
     */
    RegisterSaveStates() {
        States.Scoreboard = ScoreboardController.getState();
        States.Scorekeeper = ScorekeeperController.getState();
        States.Slideshow = SlideshowController.getState();
        States.Video = VideoController.getState();
        States.Roster = RosterController.getState();
        States.Raffle = RaffleController.getState();
        States.Capture = CaptureController.getState();
        States.Chat = ChatController.getState();
        States.Penalty = PenaltyController.getState();
        States.Sponsor = SponsorController.getState();
        States.Camera = CameraController.getState();
        ...
        SaveTimer = setInterval(() => {
            ...
            //if(!DataController.compare(state, States.Camera)) {
            //}
            //States.Camera = Object.assign({}, state);
            ...
            //if(!DataController.compare(state, States.Capture)) {
            //    DataController.saveFile(FILE_STATE_CAPTURE, JSON.stringify(state, null, 4));
            //}
            //States.Capture = Object.assign({}, state);
            
            //Chat
            //DataController.saveFile(FILE_STATE_CHAT, JSON.stringify(state, null, 4));
            //state = ChatController.getState();
            //if(!DataController.compare(state, States.Chat)) {
            //    DataController.saveFile(FILE_STATE_CHAT, JSON.stringify(state, null, 4));
            //}
            //States.Chat = Object.assign({}, state);
            ...
            //if(!DataController.compare(state, States.Penalty)) {
            //    DataController.saveFile(FILE_STATE_PENALTY, JSON.stringify(state, null, 4));
            //}
            //States.Penalty = Object.assign({}, state);
            
            //Raffle
            DataController.saveFile(FILE_STATE_RAFFLE, JSON.stringify(RaffleController.getState(), null, 4));
            //state = RaffleController.getState();
            //if(!DataController.compare(state, States.Raffle)) {
            //    DataController.saveFile(FILE_STATE_RAFFLE, JSON.stringify(state, null, 4));
            //}
            //States.Raffle = Object.assign({}, state);
            
            //Roster
            DataController.saveFile(FILE_STATE_ROSTER, JSON.stringify(RosterController.getState(), null, 4));
            //state = RosterController.getState();
            //if(!DataController.compare(state, States.Roster)) {
            //}
            //States.Roster = Object.assign({}, state);
            ...
            //if(!DataController.compare(state, States.Scoreboard)) {
            //}
            //States.Scoreboard = Object.assign({}, state);

            //Scorekeeper
            //state = ScorekeeperController.getState();
            //if(!DataController.compare(state, States.Scorekeeper)) {
            DataController.saveFile(FILE_STATE_SCOREKEEPER, JSON.stringify(ScorekeeperController.getState(), null, 4));
            //}
            //States.Scorekeeper = Object.assign({}, state);

            //Slideshow
            //state = SlideshowController.getState();
            //if(!DataController.compare(state, States.Slideshow)) {
            DataController.saveFile(FILE_STATE_SLIDESHOW, JSON.stringify(SlideshowController.getState(), null, 4));
            //}
            //States.Slideshow = Object.assign({}, state);

            //Sponsor
            //state = SponsorController.getState();
            //if(!DataController.compare(state, States.Sponsor)) {
            DataController.saveFile(FILE_STATE_SPONSOR, JSON.stringify(SponsorController.getState(), null, 4));
            //}
            //States.Sponsor = Object.assign({}, state);

            //Video
            //state = VideoController.getState();
            //if(!DataController.compare(state, States.Video)) {
                DataController.saveFile(FILE_STATE_VIDEO, JSON.stringify(VideoController.getState(), null, 4));
            //}
            //States.Video = Object.assign({}, state);
     ...
     */
    loadFile(filename, sync) {
        return new Promise((res, rej) => {
            if(sync) {
                res(DataController.FS.readFileSync(filename));
            } else {
                DataController.FS.readFile(filename, (err, data) => {
     ...
     */
    saveFile(filename, content, sync) {
        try {
            ...
            if(sync) {
                DataController.FS.writeFileSync(filename, content);
            } else {
                DataController.FS.writeFile(filename, content, DataController._emptySaveFileCallback);
            }
            //DataController.WFA(filename, content);
        } catch(er) {
            //+show error
            //console.log(er);
        }
    ...
    SaveRecord(record) {
        ...
        switch(record.RecordType) {
            ...
            case vars.RecordType.Phase :
                ...
                records = DataController.getPhases().slice();
                type = SET_PHASES;
            break;
    ...
    async UpdateRecords(type, records) {
        ...
        if(typeof(filename) === 'string' && typeof(operation) === 'string') {
            if(records instanceof Array) {
                let items = records;
     ...
     */
    getPhases() {
        return DataController.getRecords(vars.RecordType.Phase);
    },
    ...
    getPhase(id) {
        ...
        if(phases && phases.length) {
            for(var key in phases) {
                if(parseInt( phases[key].RecordID ) === parseInt( id ) )
                    return phases[key];
            }
        }
        return null;
    ...
    getAnthemSinger(id) {
        ...
        if(records[vars.RecordType.Anthem + "-" + id])
     ...
     */
    getPeer(id) {
        var peers = DataController.getPeers();
     ...
     */
    ext(src) {
        return DataController.PATH.extname(src).toLowerCase().split('.').pop();
    },
    ...
    getTeamSkaters(id) {
        ...
        if(team === null)
            return skaters;
        var records = DataController.prepareRecordsForSaving(DataController.getRecords(vars.RecordType.Skater));
        records.forEach((skater) => {
            if(skater.Teams && skater.Teams.length >= 1) {
                skater.Teams.forEach((steam) => {
                    if(parseInt(steam.TeamID) === parseInt(id)) {
                        skaters.push(skater);
                    }
                });
            }
    ...
    compare(obj1, obj2, log) {
        if(obj1 === null || obj2 === null || obj1 === undefined || obj2 === undefined) {
            ...
            if(obj1 === undefined && obj2 === undefined)
                return true;
            if(log)
                console.log(`Null value found: ${obj1} | ${obj2}`);
            return false;
        }
    ...
    getState(type = null) {
        if(typeof(type) === "string") {
            switch(type.toLowerCase()) {
                case "scoreboard" :
                    return Object.assign({}, States.Scoreboard);
                default :
                break;
            }
        }
        return Object.assign({}, DataStore.getState());
    },
     ...
     */
    async loadFolder(path, parent, folders) {
        ...
        return new Promise((res, rej) => {
            return DataController.FS.readdir(path, {encoding:'utf8'}, async (err, files) => {
                if(err) {
                    rej(err);
                } else if(files !== null && typeof(files) === 'object') {
                    for(let file of files) {
                        let pe = new Promise((fres, frej) => {
                            return DataController.FS.stat(path + file, (er, stat) => {
                                if(er)
                                    frej(er);
    ...
    async loadFolderFiles(path) {
        return new Promise((res, rej) => {
            ...
            if(typeof(path) !== 'string' || path.indexOf(FOLDER_MEDIA) !== 0)
                ...
                DataController.FS.readdir(path, "utf8", (err, files) => {
                    if(err)
                        rej(err);
                    else {
                        let records = [];
    ...
    async showOpenDialog(options) {
        ...
        return new Promise((res, rej) => {
            return DataController.DIALOG.showOpenDialog(window.RDMGR.mainWindow, settings, names => {res(names ? names : undefined)});
        });
    },
    ...
    async uploadFile(path, dest) {
        ...
        return DataController.FS.promises.copyFile(path, dest + '/' + DataController.PATH.basename(path));
    },
    ...
    async createFolder(path) {
        return new Promise((res, rej) => {
            ...
            if(DataController.FS.existsSync(path)) {
                return res(path);
            }

            let pe = DataController.FS.promises.mkdir(path);
    ...
    buildAPI() {
        ...
        exp.get(/^\/api\/record\/([A-Z]{3})$/i, (req, res) => {
            var records = null;
            if(req.params[0]) {
                records = DataController.getRecords(req.params[0], true);
                if(records !== null && req.params[0] !== vars.RecordType.Phase) {
