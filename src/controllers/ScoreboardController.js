 ...
 * @param {object} action 
 */
function ControllerReducer(state = ScoreboardState, action) {
    switch(action.type) {
        case SET_STATE :
        ...
        case RESET_STATE :
            ...
            if(state.JamState === vars.Clock.Status.Running || state.ClockState === vars.Clock.Status.Running)
                return state;

            var phases = DataController.prepareRecordsForSaving(DataController.getPhases());
            var phase = {
                PhaseID:0,
                PhaseName:"DERBY!",
                PhaseIndex:0,
...
const ScoreboardController = {
    ...
    async IncreaseTeamScore(team, amount) {
        if(ScoreboardController.getState().JamState !== vars.Clock.Status.Running) {
            ...
            ScoreboardController.SetTeamScore(team, team.Score + amount);
        }
    },
